...
export const glossaryChapterContent = {
  en: `
# Glossary
...
作業系統的組件，控制對輸入/輸出設備的存取。

**dictionary-based encoding (基於字典的編碼)**
一種在會話期間創建字典的壓縮方法。

**difference operation (差集操作)**
對兩個集合的操作，結果是第一個集合減去兩個集合中的公共元素，或關聯式資料庫中的運算子，應用於具有相同屬性的兩個關聯。結果關聯中的元組是那些在第一個關聯中但不在第二個關聯中的元組。
...
一種組織名稱空間的方法，其中名稱定義在根在頂部的倒置樹結構中。
...
**entity–relation (E-R) model (實體-關係 (E-R) 模型)**
定義關聯式資料庫中實體及其關係的模型。

**entity–relationship (E-R) diagram (實體-關係 (E-R) 圖)**
實體-關係模型中使用的圖表。
...
可以編程的可程式化唯讀記憶體。擦除 EPROM 需要將其從電腦中移除。
...
**fetch (提取)**
指令週期的一部分，其中要執行的指令從記憶體中取出。

**fiber-optic cable (光纖電纜)**
以脈衝形式攜帶信號的介質。它由另一個玻璃或塑膠圓柱體（包層）內部的玻璃或塑膠薄圓柱體（芯）組成。

**field (欄位)**
在描述資訊中具有意義的最小命名資料單位。欄位可以是變數或常數。

**File Transfer Protocol (FTP) (檔案傳輸協定)**
TCP/IP 中用於從遠端站點傳輸檔案的應用層服務。

**finite state automaton (有限狀態自動機)**
具有預定義狀態數量的機器。

**firewall (防火牆)**
安裝在組織內部網路與網際網路其餘部分之間以提供安全性的設備。

**FireWire (火線)**
具有高速串列介面並以封包形式傳輸資料的 I/O 設備控制器。

**first in, first out (FIFO) (先進先出)**
一種演算法，其中添加到列表的第一個資料項目首先從列表中刪除。

**flat-file (扁平檔案)**
與組織中任何其他檔案無關的獨立檔案。

**floating-point representation (浮點數表示法)**
一種數字表示法，其中小數點的位置浮動以創建更好的精度。通常用於在電腦中表示實數。
...
**follwing (追蹤中)**
追蹤您認識的人以在您的主頁上查看他們的推文。

**formal parameters (形式參數)**
函數中的參數宣告，用於描述要傳遞給函數的資料類型。
...
**friend (朋友)**
在 Facebook 中，可以接收另一個使用者發布的貼文的使用者。

**friendship (朋友關係)**
在 Facebook 中，分享僅在朋友之間進行。朋友關係是一對一的互惠關係。

**front (前端)**
佇列中將被 dequeue 操作刪除的下一個元素。
...
資料可以在列表中任何位置插入或刪除的列表。

**generic domain (通用網域)**
網域名稱空間中使用通用後綴的子網域。

**Gigabit Ethernet (十億位元乙太網路)**
具有一十億位元 (1000 Mbps) 資料速率的乙太網路。
...
編寫一個程式來測試任何程式（由其哥德爾數表示）是否會終止。

**hardware (硬體)**
電腦系統的任何實體組件，如鍵盤或印表機。

**hardware abstraction layer (HAL) (硬體抽象層)**
Windows 系統中隱藏上層差異的最低層程式。

**hashed file (雜湊檔案)**
使用雜湊方法之一搜尋的檔案。

**hashed MAC (HMAC) (雜湊 MAC)**
ITU 建議使用稱為 ASN.1 的知名標準來描述公鑰分發中的憑證。

**hashing method (雜湊方法)**
存取雜湊檔案的方法。

**hashtag (主題標籤)**
一種指示推文中重要單詞的方法。它以井號 (#) 開頭。
...
一種資料庫模型，將資料組織成可以從上到下搜尋的樹狀結構。

**high-level language (高階語言)**
一種（可移植）程式語言，旨在讓程式設計師專注於應用程式，而不是特定電腦或作業系統的結構。

**high-order logic (高階邏輯)**
一種邏輯，擴展了謂詞邏輯中量詞 ∀ 和 ∃ 的範圍，以綁定謂詞和變數。
...
用於指定 Web 文件內容和格式的電腦語言：允許文字包括字體、佈局、嵌入圖形和指向其他文件的連結。
...
軟體工程中的一種模型，其中構建整個套件，每個模組僅由一個外殼組成：模組隨著套件的每次迭代而增加複雜性。

**index (索引)**
陣列中元素的位址。

**indexed color (索引色)**
點陣圖形中的一種技術，僅使用全彩的一部分來編碼每個應用程式中的顏色。

**indexed file (索引檔案)**
使用索引進行隨機存取的檔案。

**infix (中綴)**
一種算術表示法，其中運算子放置在兩個運算元之間。

**infrared waves (紅外線)**
光譜中從 300 GHz 到 400 THz 的部分，用於短距離通訊。
...
**input/output (I/O) controller (輸入/輸出 (I/O) 控制器)**
控制對輸入/輸出設備存取的設備。
...
**insertion sort (插入排序)**
一種排序演算法，其中列表未排序部分的第一個元素被插入到列表已排序部分的適當位置。

**instruction (指令)**
告訴電腦做什麼的命令。

**instruction register (指令暫存器)**
CPU 中的暫存器，在控制單元解釋之前保存指令。
...
無形的東西，如想法、發明、技術、藝術品、音樂和文學，人們可以聲稱擁有所有權。

**intelligent agent (智慧代理)**
感知其環境、從中學習並與之進行智慧互動的代理。

**interface (介面)**
要傳遞給操作或從操作回傳的公開操作或資料的列表。

**internal level (內部層級)**
資料庫中定義資料實際儲存位置的部分。

**internal node (內部節點)**
除根和葉子之外的任何樹節點：樹中間的節點。
...
TCP/IP 協定中的網路層協定，負責在網際網路上將封包從一台電腦傳輸到另一台電腦。

**Internet Protocol version 6 (IPv6) (網際網路協定第六版)**
具有擴充位址空間和重新設計的封包與協定的新 IP。
...
一種 I/O 形式，其中 CPU 在發出 I/O 命令後繼續服務其他行程，直到收到 I/O 操作完成的中斷信號。

**intersection operation (交集操作)**
對兩個集合的操作，其結果是包含兩個集合共有元素的集合。
...
**job scheduler (工作排程器)**
從等待移動到記憶體的工作佇列中選擇工作進行處理的排程器。

**join operation (連接操作)**
關聯式資料庫中的一種操作，獲取兩個關聯並基於共同屬性將它們組合起來。
...
一種演算法，其中最後添加到列表的資料項目首先從列表中刪除。

**leaf (葉)**
具有一個傳入弧且沒有傳出弧的圖或樹節點。

**Lempel Ziv (LZ) encoding (Lempel Ziv (LZ) 編碼)**
一種使用字典的壓縮演算法。

**Lempel Ziv Welch (LZW) encoding (Lempel Ziv Welch (LZW) 編碼)**
LZ 編碼的增強版本。
...
**linked list (鏈結串列)**
一種線性串列結構，其中元素的順序由連結欄位決定。

**linked list resolution (鏈結串列解決)**
雜湊中的一種碰撞解決方法，使用單獨的區域來存放同義詞，這些同義詞在鏈結串列中維護。

**Linux**
由 Linus Torvalds 開發的作業系統，旨在使 UNIX 在 Intel 微處理器上運行時更有效率。
...
**loop (迴圈)**
在程式中，導致一個或多個陳述式重複的結構化程式設計建構。在圖形中，以同一頂點開始和結束的線。
...
**main memory (主記憶體)**
電腦的主記憶體，由中速隨機存取記憶體組成。與*快取記憶體*相對。
...
**medium access control (MAC) address (媒體存取控制 (MAC) 位址)**
見*資料連結層位址*。
...
一種允許在處理時多個程式駐留在記憶體中的技術。

**multithreading (多執行緒)**
一些語言（如 Java）支援的平行處理。

**mutual exclusion (互斥)**
由作業系統強加的條件，其中只有一個行程可以持有資源。
...
**neuron (神經元)**
負責在人腦和神經系統中傳輸資訊的單個細胞。

**new master file (新主檔)**
更新檔案時從舊主檔創建的主檔。

**news (新聞)**
Facebook 允許您為朋友發布新聞（稱為動態更新）。這可以是一條長訊息（最多 60 000 個字元）、指向網頁的連結、照片或影片。

**no preemption (無搶佔)**
作業系統無法暫時分配資源的情況。

**node (節點)**
在資料結構中，包含用於處理資料結構的資料和結構元素的元素。
...
**operator (運算子)**
表示對資料（運算元）採取行動的語法符號。與*運算元*相對。

**optical storage device (光學儲存設備)**
使用（雷射）光儲存和檢索資料的 I/O 設備。

**OR operator (OR 運算子)**
僅當兩個輸入都為 0 時輸出為 0，否則為 1 的二元運算。
...
**partitioning (分割)**
用於多重程式設計的技術，將記憶體劃分為可變長度的區段。

**Pascal**
一種程式語言，旨在透過強調結構化程式設計方法來教導新手程式設計。

**pass by reference (傳參考)**
一種參數傳遞技術，其中被呼叫函數使用別名引用傳遞的參數。

**pass by value (傳值)**
一種參數傳遞技術，其中變數的值被傳遞給函數。
...
**Pipelining (管線化)**
現代電腦使用一種稱為管線化的技術來提高吞吐量（在每個時間段內執行的指令總數）。其想法是，如果控制單元可以同時執行這些階段中的兩個或三個，則下一個指令可以在前一個指令完成之前開始。
...
一種加密法，明文中的字元將根據其在文本中的位置更改為密文中的不同字元。

**polycarbonate resin (聚碳酸酯樹脂)**
在 CD-ROM 生產中，注入模具的材料。

**polymorphism (多型)**
在 C++ 中，定義幾個具有相同名稱的操作，這些操作在相關類別中可以做不同的事情。
...
**postfix (後綴)**
一種算術表示法，其中運算子放置在其運算元之後。

**postorder traversal (後序遍歷)**
一種二元樹遍歷方法，其中先處理左子樹，然後是右子樹，然後是根。

**pragmatic analysis (語用分析)**
分析句子以透過消除歧義來找出單詞的真正含義。

**predicate logic (謂詞邏輯)**
一種邏輯系統，其中量詞可以應用於項但不能應用於謂詞。
...
**prefix (前綴)**
一種算術表示法，其中運算子放置在運算元之前。

**preorder traversal (前序遍歷)**
一種二元樹遍歷，其中先遍歷左子樹，接著遍歷根，最後遍歷右子樹。
...
**privacy (隱私)**
今天，私人和公共機構收集了大量關於公民的個人資訊。一些國家對這些資訊的使用有道德規範。
...
CPU 中的暫存器，保存記憶體中要執行的下一條指令的位址。

**programmable read-only memory (PROM) (可程式化唯讀記憶體)**
內容由製造商以電子方式設定且可能由使用者重置的記憶體。
...
**replaying (重播)**
一種對資訊完整性的攻擊類型，其中攻擊者攔截訊息並再次重新發送。
...
**router (路由器)**
在前三層 TCP/IP 層運行的設備，連接獨立網路。路由器根據其目的位址路由封包。

**routing (路由)**
路由器執行的過程。

**row-major storage (以列為主儲存)**
一種在記憶體中儲存陣列元素的方法，其中元素逐行儲存。

**RSA cryptosystem (RSA 密碼系統)**
由 River、Shamir 和 Adleman 設計的常見公共密碼系統。它使用兩個指數 e 和 d，其中第一個是公開的，第二個是私有的。

**RSA cryptosystem (RSA 密碼系統)**
RSA 密碼系統是常見的公鑰演算法之一。

**rule-based system (規則型系統)**
一種知識表示系統，使用一組規則從已知事實推斷新事實。

**run-length encoding (連長編碼)**
一種無損壓縮方法，其中相同符號的序列被符號和重複次數所取代。
...
**scanning (掃描)**
透過在均勻間隔的點取樣其密度和顏色將圖像轉換為數位資料。

**scheduler (排程器)**
將工作從一種狀態移動到另一種狀態的程式。

**scheduling (排程)**
將作業系統的資源分配給不同的程式，並決定哪個程式應該使用哪個資源，以及何時使用。

**scheme**
LISP 語言的事實標準。

**scientific notation (科學記數法)**
小數點左側有一位數字的數字表示法，10 的冪定義小數點的移位。

**search space (搜尋空間)**
可以透過搜尋方法檢查以找到解決方案的可能情況集合。

**searching (搜尋)**
檢查列表以定位包含稱為搜尋參數的指定值的一個或多個元素的過程。
...
**segmentation (分割)**
影像處理中的一個步驟，將圖像劃分為同質的區段或區域。

**select operation (選擇操作)**
關聯式資料庫中基於標準選擇一組元組的操作。

**selection (選擇)**
結構化程式設計中的三個建構之一。

**selection sort (選擇排序)**
一種排序演算法，其中選擇列表未排序部分中的最小值並將其放置在列表已排序部分的末尾。

**semantic analysis (語意分析)**
分析句子中單詞或陳述式中符記的含義。

**semantic network (語意網路)**
其中節點代表物件且邊代表物件之間關係的圖形。

**sequence (循序)**
結構化程式設計中的三個建構之一。

**sequential access (循序存取)**
一種存取方法，其中檔案中的記錄從第一個元素開始串列存取。

**sequential file (循序檔案)**
一種檔案結構，其中資料必須從檔案中的第一個元素開始按順序處理。

**sequential search (循序搜尋)**
與線性串列一起使用的搜尋技術，其中搜尋從第一個元素開始，一直持續到找到等於所尋找值的元素，或者到達列表的末尾。

**server (伺服器)**
在主從式系統中，提供輔助服務的中央電腦（伺服器程式）。
...
**side effect (副作用)**
由表達式求值導致的變數變化：由被呼叫函數執行的任何輸入/輸出。
...
一種整數表示方法，其中 1 位元表示數字的符號，其餘位元表示數值。
...
一種倫理原則，即如果社會上大多數人同意一項行為，那麼該行為就是合乎倫理的。
...
自然語言處理（在 AI 中）的第一步，其中分析語音信號並提取其中包含的單詞序列。
...
**starvation (饑餓)**
作業系統運作中的一個問題，其中行程無法存取它們所需的資源。

**state chart (狀態圖)**
類似於狀態圖，但用於物件導向軟體工程的圖表。
...
**string (字串)**
字串作為一組字元，在不同的語言中處理方式不同。在 C 中，字串是字元陣列。在 C++ 中，字串可以是字元陣列，但有一個名為 string 的類型。在 Java 中，字串是一種類型。
...
由多種介質和多個交換器組成的複雜 WAN。

**symbolic language (符號語言)**
一種電腦語言，與機器語言相差一級，每個機器指令都有一個助記符標識符，並且可以使用符號資料名稱。

**symmetric-key cipher (對稱金鑰加密法)**
一種密碼學類型，其中單個秘密金鑰用於加密和解密。
...
**testability (可測試性)**
軟體的一種屬性，衡量軟體作為操作系統進行測試的難易程度。

**testing phase (測試階段)**
軟體生命週期的一個階段，進行實驗以證明軟體套件有效。
...
**thresholding (閾值化)**
影像分割中使用的一種方法，其中選擇具有特定強度的像素，並且該方法試圖找到所有具有相同強度的像素。
...
一種對機密性的攻擊類型，其中攻擊者透過監控線上流量獲取一些資訊。
...
軟體系統的一種品質，指將系統從一個平台移動到另一個平台的能力。

**translator (翻譯器)**
任何語言轉換程式的通用術語。另見*組譯器*和*編譯器*。
...
**Trojan horse (特洛伊木馬)**
可以執行惡意行為（如刪除或損壞檔案）的程式。

**True-Color (全彩)**
點陣圖形中的一種技術，使用 24 位元來表示顏色。

**truncation error (截斷誤差)**
使用浮點數表示法儲存數字時發生的誤差。儲存的數字值可能不完全如預期。

**truth table (真值表)**
列出所有可能的邏輯輸入組合及相應邏輯輸出的表。
...
**Turing test (圖靈測試)**
艾倫·圖靈設計的一種測試，用於確定電腦是否可以說是真正的智慧。

**tweet (推文)**
Twitter 使用者交換的短訊息（最多 140 個字元）。發送推文。
...
二進位數字的一種表示法，其中數字的補數是透過對所有位元取補數然後加 1 找到的。

**two’s complement representation (二的補數表示法)**
一種整數表示方法，其中負數透過保持所有最右邊的 0 和第一個 1 不變並對其餘位元取補數來表示。
...
**unfollow (取消追蹤)**
使用者停止在 Twitter 上追蹤另一個使用者的過程。

**unguided media (非導引介質)**
沒有物理邊界的傳輸介質。

**Unicode (萬國碼)**
一種 32 位元代碼，包括世界上大多數語言的符號和字母。
...
對兩個集合的操作，結果包含兩個集合中的所有元素，沒有重複。

**universal serial bus (USB) (通用序列匯流排)**
連接較慢設備（如鍵盤和滑鼠）到電腦的串列 I/O 設備控制器。

**UNIX**
在電腦程式設計師和電腦科學家中流行的作業系統。

**unsigned integer (無符號整數)**
沒有符號的整數，其值範圍在 0 到正無窮大之間。
...
**utilization (功利主義)**
倫理學原則，即如果一項行為能產生好的結果，那麼它就是合乎倫理的。

**utilization (功利主義)**
第二個倫理理論，與行為的後果有關。如果一項行為導致對社會有用的後果，那麼它就是合乎倫理的。

**variable (變數)**
在程式執行期間其值可以更改的記憶體儲存物件。與*常數*相對。

**vector graphic (向量圖形)**
一種類型的圖形檔案格式，其中線條和曲線使用數學公式定義。
...
一種記憶體組織形式，允許在記憶體和磁性儲存之間交換程式，給人一種比實際存在的主記憶體更大的印象。

**virus (病毒)**
隱藏在另一個程式內部的不需要的程式，它可以自我複製。
...
**Windows NT**
由 Microsoft 設計用於取代 MS-DOS 的作業系統。
...
**Worldwide Interoperability Access (全球互通微波存取)**
DSL 或電纜連接到網際網路的無線版本。
...
一種位元運算，僅當其中一個運算元為 1 時，運算結果才為 1。

**zero-knowledge authentication (零知識驗證)**
一種實體驗證方法，其中索賠人不洩露任何可能危及秘密機密性的東西。
`,
};
