...
export const ProblemSolver: React.FC<ProblemSolverProps> = ({ id, setView, isSidebarOpen }) => {
  ...
  // State for the card content needs to be inside the animated component,
  // or it will be shared across problem transitions. The key={id} on motion.div
  // will create a new instance each time, so local state is fine.
    ...
    <div className="relative h-full flex flex-col">
      <div className="flex-1 flex flex-col justify-center min-h-0 overflow-hidden">
        ...
        <AnimatePresence initial={false} custom={direction} mode="wait">
          ...
          >
            <div className="max-w-4xl mx-auto h-full overflow-y-auto pb-24 lg:pb-0">
              <div className="bg-[var(--bg-translucent)] backdrop-blur-xl border border-[var(--glass-border)] shadow-[var(--glass-shadow)] rounded-2xl p-4 sm:p-8 mb-6">
              ...
              <div className="bg-[var(--bg-translucent)] backdrop-blur-xl border border-[var(--glass-border)] shadow-[var(--glass-shadow)] rounded-2xl p-4 sm:p-8">
                ...
                <div className="space-y-3">
                  {problem.options.map((option) => {
                    ...
                    return (
                      <div
                        key={option.key}
                        className={`flex items-start gap-4 p-3 sm:p-4 rounded-xl border-2 transition-all ${
                          isCorrect
      ...
      <button onClick={() => navigate(-1)} disabled={problemIndex <= 0} className="hidden lg:flex items-center justify-center absolute top-1/2 -left-20 -translate-y-1/2 w-12 h-12 bg-[var(--ui-bg)] rounded-full text-[var(--text-secondary)] disabled:opacity-50 hover:bg-[var(--ui-bg-hover)] transition-colors z-20">
        <ChevronLeftIcon className="w-6 h-6" />
      </button>
      <button onClick={() => navigate(1)} disabled={problemIndex >= problems.length - 1} className="hidden lg:flex items-center justify-center absolute top-1/2 -right-20 -translate-y-1/2 w-12 h-12 bg-[var(--ui-bg)] rounded-full text-[var(--text-secondary)] disabled:opacity-50 hover:bg-[var(--ui-bg-hover)] transition-colors z-20">
