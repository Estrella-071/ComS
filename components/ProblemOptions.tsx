  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      {problem.options.map((option, index) => {
        ...
        // Use showChinese prop to determine display language.
        // Default is English (false).
        ...
        if (isRevealed) {
           if (isCorrect) {
               ...
               containerClasses += "bg-transparent border-transparent opacity-40 grayscale blur-[0.5px]";
               indicatorClasses += "bg-[var(--ui-bg)] text-[var(--text-subtle)] border-[var(--ui-border)]";
               textClasses += "text-[var(--text-secondary)]";
           }
        } else {
            if (isSelected) {
                containerClasses += "bg-[var(--text-primary)] border-[var(--text-primary)] shadow-xl transform scale-[1.01] z-10";
                indicatorClasses += "bg-[var(--bg-color)] text-[var(--text-primary)] border-[var(--bg-color)]";
                textClasses += "text-[var(--bg-color)]";
            } else {
                containerClasses += "bg-[var(--bg-translucent)] border-[var(--ui-border)] hover:border-[var(--text-primary)] hover:bg-[var(--ui-bg)] cursor-pointer hover:shadow-lg hover:translate-x-1";
        ...
        return (
          <motion.button
            ...
            initial={false}
            whileTap={!disabled && !isRevealed ? { scale: 0.99 } : {}}
          >
